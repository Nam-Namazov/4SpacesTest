...
class ShopImpl: Shop {
    ...
    func addNewProduct(product: Product) -> Bool {
        return products.updateValue(product, forKey: product.id) == nil
    }
    ...
    func listProductsByName(searchString: String) -> Set<String> {
        ...
        let crossReference = Dictionary(grouping: filteredProducts, by: \.name)
        
        crossReference
            .filter { $1.count > 1 }
            .values
            .forEach { $0.forEach { result.insert("\($0.producer) - \($0.name)") } }
        
        crossReference
            .filter { $1.count == 1 }
            .values
            .forEach { $0.forEach { result.insert($0.name) } }
        
        return result
    }
    
    func listProductsByProducer(searchString: String) -> [String] {
        let filteredProductNames = products
